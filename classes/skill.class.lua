...
CSkill = class(
	function (self, copyfrom)
		...
		self.MaxEngeryPer = 100;	-- Must have less than this % Mana to use
		self.MinEnergyPer = 0;	-- Must have more then this % Mana to use
		...
		if( type(copyfrom) == "table" ) then
			...
			self.EnergyType = copyfrom.EnergyType;
			self.EnergyValue = copyfrom.EnergyValue;
			...
			self.PlayerParalyzed =  copyfrom.PlayerParalyzed;
			self.Playerdead = copyfrom.Playerdead ;
			self.PlayerItem =  copyfrom.PlayerItem;
			self.PlayerState = copyfrom.PlayerState;
...
function CSkill:canUse(_only_friendly, target)
	...
	if skills.funcs["skills_noskill_zone"](self) then
		debug_skilluse("NOSKILLZONE")
		return false
	end
	...
	if skills.funcs["skills_need_buff_2"](self) then
...
--	if( (player.Battling or player.Fighting) and self.InBattle == false ) then
	...
	if self.Consumable then
		local ammo = skills.funcs["skills_ammo_slot"](self);
	...
	if self.Type == STYPE_SUMMON then
		if skills.funcs["skills_pet_summon"](self) then
	...
	if skills.funcs["skills_pet_heal"](self) then
		debug_skilluse("USINGPETHEAL");
		return false
	end
...
--	self.LastCastTime = os.time() + self.CastTime;
	...
	--=== warden usage
	if skills.funcs["skills_use_summon"](self) then
		return true
	end
